<div class="p-6 bg-white rounded-lg shadow-md">
    <h3 class="mb-6 text-lg font-semibold text-gray-800">編輯折價</h3>
    <form method="POST" action="{{ route('admin.discounts.update', $discount->discount_id) }}">
        ...
        <div class="grid grid-cols-1 gap-4 mb-4 md:grid-cols-2">
            <div>
                ...
                <input type="text" name="discount_name" id="discount_name" value="{{ old('discount_name', $discount->discount_name) }}"
            ...
            <div>
                ...
                <select name="discount_type" id="discount_type" class="mt-1 w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('discount_type') border-red-500 @enderror">
                    <option value="percentage" {{ old('discount_type', $discount->discount_type) == 'percentage' ? 'selected' : '' }}>百分比</option>
                    <option value="fixed_amount" {{ old('discount_type', $discount->discount_type) == 'fixed_amount' ? 'selected' : '' }}>固定金額</option>
                </select>
                @error('discount_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="discount_value" id="discount_value" value="{{ old('discount_value', $discount->discount_value) }}"
        ...
        <div class="mb-4">
            ...
            <textarea name="discount_description" id="discount_description" rows="4"
                      class="mt-1 w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('discount_description') border-red-500 @enderror">{{ old('discount_description', $discount->discount_description) }}</textarea>
        ...
        <div class="mb-4">
            ...
            <select name="products[]" id="products" multiple
                ...
                @foreach ($products as $product_id => $product_name)
                    <option value="{{ $product_id }}" {{ in_array($product_id, old('products', $selectedProducts)) ? 'selected' : '' }}>{{ $product_name }}</option>
                @endforeach
            </select>
            @error('products')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div class="flex gap-2">
            <button type="submit" class="px-4 py-2 text-white transition duration-200 bg-blue-600 rounded-lg hover:bg-blue-700">更新</button>
