@extends('layouts.app', ['page_name' => '編輯商品', 'show_footer' => false, 'show_header' => false])

@section('content')
    <div class="min-h-screen p-4 bg-gray-50 sm:p-6 lg:p-8">
        <div class="max-w-lg p-6 mx-auto bg-white rounded-lg shadow-md">
            <h3 class="mb-4 text-lg font-semibold text-gray-800">編輯商品</h3>
            <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="product_description" id="product_description"
                              class="block w-full px-3 py-2 mt-1 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('product_description', $product->product_description) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    @error('product_image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <div id="image-preview" class="mt-2 {{ $product->product_image ? '' : 'hidden' }}">
                        <img id="preview-img" src="{{ $product->product_image ? asset('images/products/' . $product->product_image . '.png') : '#' }}"
                             alt="商品圖片預覽" class="h-auto max-w-full rounded-lg shadow-sm" style="max-height: 200px;">
                    </div>
                    @if ($product->product_image)
                        <p class="mt-1 text-sm text-gray-600">目前圖片：{{ $product->product_image }}</p>
                    @endif
                </div>
                <div class="mb-4">
    ...
    <script>
        document.getElementById('product_image').addEventListener('change', function(event) {
            ...
            if (file) {
                ...
                previewImg.src = "{{ $product->product_image ? asset('storage/images/products/' . $product->product_image) : '#' }}";
                previewContainer.classList.toggle('hidden', !previewImg.src.includes('storage'));
            }
        });
    </script>
@endsection
