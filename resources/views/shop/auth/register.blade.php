@extends('layouts.app', ['page_name' => '註冊帳號', 'show_footer' => false, 'show_header' => false])

@section('content')
    <div class="flex items-center justify-center min-h-screen p-4 bg-gray-50">
        <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
            <h2 class="mb-6 text-2xl font-bold text-center text-gray-800">註冊帳號</h2>
            <form action="{{ route('user.register.post') }}" method="POST">
                ...
                <div class="flex justify-end">
                    <button type="submit"
                            class="w-full px-4 py-2 text-white transition-colors bg-blue-600 rounded-lg hover:bg-blue-700">註冊</button>
                </div>
            </form>
        </div>
    </div>
